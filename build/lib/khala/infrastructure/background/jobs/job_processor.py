...
class JobProcessor:
    ...
    async def _store_job_redis(self, job: JobDefinition) -> None:
        ...
        # Set TTL
        await self.redis_client.expire(f"job:{job.job_id}", self.redis_ttl_seconds)
        await self.redis_client.expire("job:queue", self.redis_ttl_seconds)
    
    async def _get_next_job(self) -> Optional[JobDefinition]:
        """Get next job from queue."""
        if self.redis_client:
            # Get from priority queue
            job_key = await self.redis_client.zpopmin("job:queue")
            if job_key:
                job_key = job_key.decode() if isinstance(job_key, bytes) else job_key
    ...
    async def _store_result(self, result: JobResult) -> None:
        ...
        if self.redis_client:
            ...
            await self.redis_client.expire(f"result:{result.job_id}", self.redis_ttl_seconds)
    ...
    async def get_queue_stats(self) -> Dict[str, Any]:
        ...
        if self.redis_client:
            ...
            for job_key in job_keys:
                ...
                if status and status.decode() == JobStatus.RUNNING.value:
